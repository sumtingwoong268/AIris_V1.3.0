...
function AIReportHTML({ html }: { html: string }) {
  ...
  return (
    <div
      className="prose prose-neutral dark:prose-invert max-w-none my-8 rounded-3xl border border-white/60 bg-white/80 p-6 shadow-xl backdrop-blur dark:border-white/10 dark:bg-slate-900/70"
...
export default function Reports() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <main className="container mx-auto max-w-6xl space-y-10 px-6 pt-24 md:pt-28 pb-20 animate-slide-in-right md:animate-none">
        ...
        {reports.length > 0 && latestReportDetails && (
          <section className="space-y-4">
            ...
            {latestReportDetails.structured ? (
              ...
              <div className="rounded-3xl border border-white/60 bg-white/80 p-6 text-sm leading-relaxed shadow-xl backdrop-blur text-slate-700 dark:border-white/10 dark:bg-slate-900/70 dark:text-slate-100">
        ...
        {reports.length === 0 ? (
          ...
          <section className="space-y-4">
            {reports.map((report) => {
              ...
              return (
                <Card key={report.id} className="transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/10 border-white/40 bg-white/60 backdrop-blur dark:border-white/5 dark:bg-slate-900/40">
                  <CardHeader>
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <CardTitle className="flex items-center gap-2 text-slate-900 dark:text-slate-50">
                          <FileText className="h-5 w-5 opacity-90 transition-transform group-hover:scale-110" style={{ color: accentColor }} />
                          {report.title}
                        </CardTitle>
                      ...
                      <Button size="sm" variant="outline" className="border-primary/20 bg-primary/5 hover:bg-primary/10 hover:text-primary dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10 dark:text-white" onClick={generatePDF} disabled={generating}>
                  ...
                  <CardContent>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-[0.1em]">
                      {urgency && (
                        <span className="rounded-full bg-slate-100 px-3 py-1 text-slate-700 ring-1 ring-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:ring-slate-700">
                          Urgency · {String(urgency).replace(/_/g, " ")}
                        </span>
                      )}
                      {traffic && (
                        <span
                          className="rounded-full px-3 py-1 ring-1 ring-inset ring-black/5"
                    ...
                    {keyFindings.length > 0 && (
                      <ul className="mt-5 grid gap-2 text-sm text-slate-700 dark:text-slate-200">
                        {keyFindings.slice(0, 3).map((finding: string, idx: number) => (
                          <li key={idx} className="flex gap-3 rounded-xl border border-white/50 bg-white/50 px-4 py-3 shadow-sm backdrop-blur transition-colors hover:bg-white/80 dark:border-white/5 dark:bg-white/5 dark:hover:bg-white/10">
                            <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" />
                            {finding}
                          </li>
                        ))}
                        {keyFindings.length > 3 && (
                          <li className="ml-5 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">
                            +{keyFindings.length - 3} more insights inside report
                          </li>
                        )}
                      </ul>
                    )}
